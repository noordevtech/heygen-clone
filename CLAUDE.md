...
- **Login (`/login`)** — sign-in page. Auth uses an opaque HTTP-only session cookie (`studio_session`) backed by a Postgres `sessions` table; sessions live 30 days. Middleware (`src/middleware.ts`) bounces unauthenticated browser navigation to `/login` and returns JSON 401 for unauthenticated `/api/*` calls. The middleware only checks cookie presence (edge runtime can't reach Postgres) — real session lookup + role gating happens in API routes and server pages via `getSessionUser()`. Bootstrap admin: `info@noordev.com` / `Idig871y@` (seeded by migration `0009_auth.sql`). All data is per-user: `app_settings` is keyed by `(user_id, key)`; `jobs` and `channels` carry a `user_id` and the UI only shows the signed-in user's rows. The admin's existing API keys + YouTube connection from the original deploy were backfilled to the admin's `user_id` and act as **platform defaults** — when a user hasn't configured their own copy of a key, the runtime resolver falls back to the admin's value (except for `youtube_refresh_token` + `youtube_channel_title`, which are strictly per-user — each user connects their own channel). User context is propagated through every provider helper via `AsyncLocalStorage` (`src/lib/user-context.ts`); API routes wrap their bodies in `withUser(...)` and the worker wraps each job + scheduled fire in `runWithUser(channel.userId, …)`.

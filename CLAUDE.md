...
- **Tasks (`/tasks`)** — channel CRUD: name, niche, schedule (Daily/Weekly/Monthly), run time (HH:MM, **server timezone — UTC on Railway**), target length, style, **YouTube connection to publish to**. Each channel auto-fires at its `runTime` via a 60s scheduler tick inside the worker process; the **Fire now** button is the same entry point on demand. The full pipeline: brainstorm 5 ideas → Claude picks best → write script → plan scenes → Pexels → longform render (no title card) → SEO + OpenRouter thumbnail → upload to the channel's selected YouTube connection (Education category, `en` default + audio language) → SRT captions in English. A user can connect multiple YouTube channels (`youtube_connections` table, migration `0011`) and each Task picks one via `channels.youtube_connection_id`.
- **Admin (`/admin`)** — admin-only user management. Create accounts, toggle `active` (inactive accounts can't log in), promote/demote between `user` and `admin`, reset passwords (revokes existing sessions), delete users. The admin can't deactivate / demote / delete themselves.
- **Login (`/login`)** — sign-in page. Auth uses an opaque HTTP-only session cookie (`studio_session`) backed by a Postgres `sessions` table; sessions live 30 days. Middleware (`src/middleware.ts`) bounces unauthenticated browser navigation to `/login` and returns JSON 401 for unauthenticated `/api/*` calls. The middleware only checks cookie presence (edge runtime can't reach Postgres) — real session lookup + role gating happens in API routes and server pages via `getSessionUser()`. Bootstrap admin: `info@noordev.com` / `Idig871y@` (seeded by migration `0009_auth.sql`). All data is strictly per-user: `app_settings` is keyed by `(user_id, key)`; `jobs` and `channels` carry a `user_id` and the UI only shows the signed-in user's rows. Each user must configure their own API keys + YouTube connection — there's no fallback to the admin's values. The admin's existing keys (backfilled in `0009`) remain visible only to the admin. User context propagates through every provider helper via `AsyncLocalStorage` (`src/lib/user-context.ts`); API routes wrap their bodies in `withUser(...)` and the worker wraps each job + scheduled fire in `runWithUser(ownerId, …)`.
...
1. **Per-user data scoping.** Auth + admin user management are live (see Login/Admin sections above), but `jobs` and `channels` are still global — every authenticated user sees every other user's runs. `user_id` columns already exist on those tables (backfilled to the admin); the remaining work is to write `user_id` from the session on create and filter queries by it.

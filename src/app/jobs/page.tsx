...
export default async function JobsPage() {
  ...
  return (
    <main className="space-y-6">
      ...
      {jobs.length === 0 ? (
        <div className="card p-6 text-[#9aa0b4]">No jobs yet. Head to the studio to create one.</div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {jobs.map((j) => (
            <div key={j.id} className="card p-4 space-y-3">
              ...
              <div className="text-sm line-clamp-3 text-[#c8cce0]">{j.request.script}</div>
              {j.videoUrl && (
